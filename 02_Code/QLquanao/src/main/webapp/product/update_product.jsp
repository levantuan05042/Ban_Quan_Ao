...
<div class="content">
  <div class="container">
    ...
    <form action="/products?action=updatePost" method="post">
      ...
      <div class="mb-3">
        ...
        <select class="form-control" name="categoryid" id="productCategory" required>
          ...
          <c:forEach items="${productCategories}" var="category">
            <option value="${category.id}" ${category.id == product.productCategory.id ? 'selected' : ''}>${category.categoryname}</option>
          </c:forEach>
        </select>
      </div>
      <div class="mb-3">
        ...
        <select class="form-control" name="supplierId" id="supplier" required>
          ...
          <c:forEach items="${suppliers}" var="s">
            <option value="${s.id}" ${s.id == product.supplier.id ? 'selected' : ''}>${s.supplierName}</option>
          </c:forEach>
        </select>
      </div>
      <div class="mb-3">
      ...
      <div class="d-flex justify-content-between">
        <button type="submit" class="btn btn-primary" onclick="return confirm('Bạn có chắc chắn muốn sửa sản phẩm này không?');">
