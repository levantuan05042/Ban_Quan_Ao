...
<div class="content">
    <div class="banner">
        <h2>Chào Mừng Đến Với Cửa Hàng Quần Áo!</h2>
    </div>

    <div class="container">
        <h2 class="mb-4 text-center"><i class="fas fa-tshirt"></i>Danh Sách Sản Phẩm</h2>
        <a href="products?action=createGet" class="btn btn-primary mb-3">
        ...
        <table class="table table-hover">
            ...
            <tbody>
            <c:forEach items="${products}" var="p" varStatus="pro">
                <tr>
                    <td>${pro.count}</td>
                    <td>${p.id}</td>
                    <td>${p.productname}</td>
                    <td>${p.price}</td>
                    <td>
                        <a href="products?action=findByGetProductCategory&categoryid=${p.productCategory.id}" class="text-decoration-none">
                                ${p.productCategory.categoryname}
                        </a>
                    </td>
                    <td>
                        <a href="supplier?action=view&id=${p.supplier.id}" class="text-decoration-none">
                    ...
                    <td>${p.entrydate}</td>
                    <td>
                        <a href="sizes?action=findBySize&productid=${p.id}" class="btn btn-info btn-sm">
                            <i class="fas fa-eye"></i> Kích Cỡ
                        </a>
                        <a href="products?action=updateGetproduct&id=${p.id}" class="btn btn-warning btn-sm">
