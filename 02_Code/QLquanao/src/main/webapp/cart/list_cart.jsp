...
<div class="sidebar">
  ...
  <a href="/carts"class="custom-link"><i class="fas fa-shopping-cart icon"></i> Giỏ Hàng</a>
...
<div class="content">
  <div class="container">
    <h2 class="mb-4 text-center"><i class="fas fa-shopping-cart"></i>Giỏ Hàng</h2>
    <table class="table table-hover table-bordered">
      ...
      <tbody>
      <c:forEach items="${carts}" var="c" varStatus="ca">
        <tr>
          <td>${ca.count}</td>
          <td>${c.id}</td>
          <td>
            <a href="accounts?action=view&username=${c.accountName}" class="text-decoration-none">
                ${c.accountName}
            </a>
          </td>
          <td>${c.fullName}</td>
          <td>
            <a href="products?action=view&productName=${c.productName}" class="text-decoration-none">
                ${c.productName}
            </a>
          </td>
          ...
          <td>${c.sizeName}</td>
          <td>
            <img src="${pageContext.request.contextPath}/uploads/${c.productimage}" alt="${c.productimage}">
          </td>
          <td>${c.quantity}</td>
          <td>${c.price}</td>
          <td>${c.total}</td>
